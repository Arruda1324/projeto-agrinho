...
function setupRecaptcha() {
    const recaptchaBox = document.querySelector('.recaptcha-checkbox');
    const recaptchaChallenge = document.getElementById('recaptcha-challenge');
    ...
    recaptchaBox.addEventListener('click', function() {
        ...
        // Simula verificação (pode substituir por lógica real depois)
        const isSimpleVerification = Math.random() > 0.3;
        
        if (isSimpleVerification) {
            // Verificação simples - apenas marca o checkbox
            checkbox.checked = true;
            this.classList.add('verificado');
        } else {
            // Leva para o desafio de imagens
            if (recaptchaChallenge) {
                recaptchaChallenge.style.display = 'block';
                
                // Configurar desafio
                setupRecaptchaChallenge(() => {
                    checkbox.checked = true;
                    this.classList.add('verificado');
                    recaptchaChallenge.style.display = 'none';
                });
            }
        }
    });
...
// Configuração do desafio de reCAPTCHA com imagens
function setupRecaptchaChallenge(onSuccess) {
    const images = document.querySelectorAll('.recaptcha-image');
    const verifyButton = document.querySelector('.btn-recaptcha');
    ...
    if (verifyButton) {
        verifyButton.addEventListener('click', function() {
            // Simples simulação (aceita qualquer seleção)
            onSuccess();
        });
    }
}
